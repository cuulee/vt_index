...
type Polygon_Index struct {
	Latconst float64
	Index map[string][]Yrow
}
...
func (p_index Polygon_Index) Pip_Simple(pt []float64) string {
	ycollisions := p_index.Index[geo.NewPoint(pt[0], p_index.Latconst).GeoHash(9)]
	for _, i := range ycollisions {
		if (i.Range[0] <= pt[1]) && (i.Range[1] >= pt[1]) {
			return i.Area
		}
